...
public class AdminWorkspaceServlet extends HttpServlet {
    ...
    private void renderDetails(HttpServletResponse response, String contextPath, IgTopic topic,
        ...
        try (PrintWriter out = response.getWriter()) {
            AdminShellRenderer.render(out, "Workspace Details - InteropHub", contextPath, panelOut -> {
                panelOut.println("      <section class=\"panel\">");
                panelOut.println("        <h2>Workspace Details</h2>");
                panelOut.println("        <p>This view is read-only. Use Edit to change this workspace.</p>");
                ...
                panelOut.println(
                        "        <p><a href=\"" + contextPath + "/admin/workspaces?workspaceId="
                                + workspace.getWorkspaceId()
                                + "&mode=edit\">Edit Workspace</a></p>");
                panelOut.println(
                        "        <p><a href=\"" + contextPath + "/admin/topics?topicId=" + workspace.getTopicId()
                                + "\">Back to Topic</a></p>");
        ...
        try (PrintWriter out = response.getWriter()) {
            AdminShellRenderer.render(out,
                    ...
                    contextPath, panelOut -> {
                        panelOut.println("      <section class=\"panel\">");
                        panelOut.println(
                                "        <h2>" + (createNew ? "Create Workspace" : "Edit Workspace") + "</h2>");
                        panelOut.println("        <p><strong>Topic:</strong> "
                        ...
                        out.println(
                                "      <input id=\"workspaceName\" name=\"workspaceName\" type=\"text\" required value=\""
                        ...
                        if (!createNew && workspace.getWorkspaceId() != null) {
                            ...
                            for (int i = 0; i < workspaceSteps.size(); i++) {
                                ...
                                out.println(
                                        "              <button type=\"submit\" name=\"stepAction\" value=\"move-down:"
                            ...
                            out.println("      </table>");
                            out.println(
                                    "      <p>Existing steps can be renamed and reordered. Steps cannot be deleted.</p>");
                        } else {
    ...
    private void renderError(HttpServletResponse response, String contextPath, String message) throws IOException {
        ...
        try (PrintWriter out = response.getWriter()) {
            AdminShellRenderer.render(out, "Workspace Error - InteropHub", contextPath, panelOut -> {
                panelOut.println("      <section class=\"panel\">");
                panelOut.println("        <h2>Workspace Request Error</h2>");
    ...
    private void renderForbidden(HttpServletResponse response, String contextPath) throws IOException {
        ...
        try (PrintWriter out = response.getWriter()) {
            AdminShellRenderer.render(out, "Access Denied - InteropHub", contextPath, panelOut -> {
                ...
                panelOut.println("        <p>You must be an InteropHub admin to access workspace settings.</p>");
