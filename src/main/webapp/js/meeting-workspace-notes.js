...
.tippy-box[data-animation=fade][data-state=hidden] {
  ...
  // frontend/topic-note-node-id-extension.js
  ...
  var TopicNoteNodeIdExtension = Extension.create({
    name: "topicNoteNodeId",
    addProseMirrorPlugins() {
      return [
        new Plugin({
          appendTransaction(transactions, oldState, newState) {
            if (!transactions.some(function(transaction2) {
              return transaction2.docChanged;
            })) {
              return null;
            }
  ...
  // frontend/topic-note-editor.js
  var TopicNoteListItem = ListItem.extend({
    addAttributes() {
      return {
        nodeId: {
          default: null,
  ...
  function buildEmptyBulletDocument() {
    return {
      type: "doc",
      content: [
        {
  ...
  function createTopicNoteEditor(options) {
  ...
  // frontend/meeting-workspace-notes.js
  (function() {
    ...
    function startRealtime(state) {
      ...
      source.addEventListener("editor-changed", function(event) {
        ...
        if (hadEditControl && !state.canEdit) {
          ...
          setMessage(
            state,
            (state.activeEditorDisplayName || "Another user") + " is now taking notes. Your local unsaved copy remains visible but was not saved.",
            "danger"
          );
        }
      });
    ...
    function handleMetadataEvent(state, payload) {
      ...
      state.canEdit = Boolean(state.currentUserId) && Boolean(state.activeEditorUserId) && Number(state.currentUserId) === Number(state.activeEditorUserId) && state.noteStatus !== "FINALIZED" && state.meetingStatus !== "CLOSED";
      state.canAssumeEditor = Boolean(state.config.canAssumeEditor) && state.noteStatus !== "FINALIZED" && state.meetingStatus !== "CLOSED";
      syncOwnerText(state);
      syncControlVisibility(state);
    }
    function startFallbackPollingIfNeeded(state) {
      ...
      const intervalMs = Number(state.config.pollIntervalMs || 5e3);
    ...
    function assumeEditorship(state, openEditorAfter) {
      ...
      const url = state.noteId ? withQueryParam(state.config.takeOverUrl, "noteId", state.noteId) : state.config.startEditingUrl;
      if (!url) {
        return;
      }
      ...
      fetch(url, {
      ...
      }).then(readJsonEnvelope).then(function(result) {
        ...
        if (result.json.document && !state.isDirty) {
          state.lastSavedDocument = cloneJson2(result.json.document);
    ...
    function enterEditMode(state) {
      ...
      const editorContent = cloneJson2(state.lastSavedDocument);
      if (!state.editor) {
    ...
    function closeEditor(state) {
      ...
      clearRetry(state);
      if (state.editor) {
        state.editor.commands.setContent(cloneJson2(state.lastSavedDocument), false);
        setEditorEditable(state, state.canEdit);
      }
      state.isDirty = false;
      toggleDirtyIndicator(state);
    ...
    function executeSave(state, mode) {
      ...
      fetch(state.config.saveUrl, {
        ...
        state.config.revision = state.revision;
        state.config.editorVersion = state.editorVersion;
        state.lastSavedDocument = cloneJson2(payload.document);
        state.isDirty = false;
        state.retryIndex = 0;
        toggleDirtyIndicator(state);
        renderReadOnly(state, state.lastSavedDocument);
    ...
    function preserveVisibleDraft(state) {
      if (state.editor) {
        const localDraftDocument = cloneJson2(state.editor.getJSON());
    ...
    function pollEditorState(state) {
      ...
      fetch(url, { method: "GET" }).then(readJsonEnvelope).then(function(result) {
        if (!result.response.ok || !result.json.ok) {
          return;
        }
        ...
        const previousRevision = state.revision;
        handleMetadataEvent(state, result.json);
        const editorChanged = previousEditorVersion !== state.editorVersion || previousActiveEditor !== state.activeEditorUserId;
    ...
    function refreshSavedContent(state, sinceRevision) {
      ...
      const baselineRevision = typeof sinceRevision === "number" ? sinceRevision : Math.max(0, Number(state.revision || 0) - 1);
    ...
    function loadOutcomes(state) {
      ...
      return fetch(url, { method: "GET" }).then(readJsonEnvelope).then(function(result) {
        if (!result.response.ok || !result.json.ok) {
          return;
        }
    ...
    function nextRetryDelay(state) {
      const schedule = [2e3, 5e3, 1e4, 3e4];
    ...
    function renderReadOnly(state, documentJson) {
      ...
      state.readOnlyEditor = createTopicNoteEditor({
        ...
        editable: false,
        content: cloneJson2(documentJson)
      });
      applyOutcomeMarkers(state);
    }
    function syncOwnerText(state) {
      ...
      if (state.noteStatus === "FINALIZED" || state.meetingStatus === "CLOSED") {
        ...
        message = (state.activeEditorDisplayName || "User #" + state.activeEditorUserId) + " is taking notes for this topic.";
      }
